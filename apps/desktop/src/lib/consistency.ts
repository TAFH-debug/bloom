...
export type DayScore = {
  date: string;
  /** Average completion ratio for habits due that day, or null if none were due. */
  score: number | null;
};

export function computeDailyScores(params: {
  habits: HabitForScore[];
  completions: CompletionForScore[];
  windowDays?: number;
  end?: Date;
  ...
  return keys.map((date) => {
    ...
    const ratios = habits
      .map((habit) => habitRatioOnDay(habit, date, completions))
      .filter((value): value is number => value !== null);
